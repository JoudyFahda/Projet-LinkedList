...
LinkedList:: LinkedList(const vector<short>& vect= {}){
    ...
    for (size_t i=1; i<vect.size();i++){
        ...
        //node1 -> next = node;
        //node1 = node;
        node1->next = make_shared<ListNode>(ListNode(vect[i]));
        node1=node1->next;
    }
...
LinkedList::LinkedList(const LinkedList& liste){
    ...
    //head= newnode; 
    head=make_shared<ListNode> (ListNode(liste.head->val));
    ...
    shared_ptr<ListNode> savedPtr=head;
    shared_ptr<ListNode> nextNodePtr=liste.head;
    for (size_t i=0; i<liste.size; i++){
       // newnode= new ListNode(nextNodePtr -> val); //on reinitialise notre newnode, il va pointer au node suivant dans la liste.
        savedPtr->next = make_shared<ListNode>(ListNode(nextNodePtr->next->val));
...
bool LinkedList::operator==(const LinkedList& liste)const{
    ...
    shared_ptr<ListNode> first = head;
    shared_ptr<ListNode> second = liste.head;
...
void LinkedList :: push_back(short data) {
    ...
    current->next =make_shared<ListNode> (ListNode(data));
...
void LinkedList :: push_front(short data) {
    ...
    head=make_shared<ListNode> (ListNode(data,head));
    // newHead->next = head;
    // head = newHead;
    ++size;
}
